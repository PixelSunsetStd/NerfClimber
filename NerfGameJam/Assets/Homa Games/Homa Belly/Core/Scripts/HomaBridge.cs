    ...
    public class HomaBridge : IHomaBellyBridge
    {
        ...
        public void TrackResourceEvent(ResourceFlowType flowType, string currency, float amount, string itemType, string itemId)
        {
            ...
            if (attributions != null)
            {
                foreach (IAttribution attribution in attributions)
                {
                    attribution.TrackEvent(flowType.ToString(), new Dictionary<string, object>()
                    {
                        { "itemId", itemId },
                        { "currency", currency },
                        { "amount", amount },
        ...
        public void TrackProgressionEvent(ProgressionStatus progressionStatus, string progression, int score = 0)
        {
            ...
            if (attributions != null)
            {
                foreach (IAttribution attribution in attributions)
                {
                    attribution.TrackEvent(progressionStatus.ToString(), new Dictionary<string, object>()
                    {
        ...
        public void TrackErrorEvent(ErrorSeverity severity, string message)
        {
            ...
            if (attributions != null)
            {
                foreach (IAttribution attribution in attributions)
                {
                    attribution.TrackEvent(severity.ToString(), new Dictionary<string, object>()
                    {
                        { "message", message }
                    });
                }
        ...
        public void TrackDesignEvent(string eventName, float eventValue = 0f)
        {
            ...
            if (attributions != null)
            {
                foreach (IAttribution attribution in attributions)
                {
                    attribution.TrackEvent(eventName, new Dictionary<string, object>()
                    {
        ...
        public void TrackAdEvent(AdAction adAction, AdType adType, string adNetwork, string adPlacementId)
        {
            ...
            if (attributions != null)
            {
                foreach (IAttribution attribution in attributions)
                {
                    attribution.TrackEvent(adAction.ToString(), new Dictionary<string, object>()
                    {
                        { "adType", adType },
                        { "adNetwork", adNetwork },
...
#region Private helpers
        ...
        /// <returns></returns>
        private List<Type> GetHomaBellyAvailableClasses(Type type)
        {
...
#region Mediators
        ...
        private void InstantiateMediators()
        {
            ...
            List<Type> availableMediators = GetHomaBellyAvailableClasses(typeof(IMediator));
...
#region Attributions
        ...
        private void InstantiateAttributions()
        {
            ...
            List<Type> availableAttributions = GetHomaBellyAvailableClasses(typeof(IAttribution));
        ...
        private void InitializeAttributions()
        {
            if (attributions != null)
            {
                ...
                foreach (IAttribution attribution in attributions)
                {
                    if (!string.IsNullOrEmpty(geryonTestingId))
                    {
...
#region Analytics
        ...
        private void InstantiateAnalytics()
        {
            ...
            List<Type> availableAnalytics = GetHomaBellyAvailableClasses(typeof(IAnalytics));
